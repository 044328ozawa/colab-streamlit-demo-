...
# タイトル部分（トップページ）
st.title("在庫管理・需要予測AIプロジェクト - データサイエンスレポートデモ")

st.markdown("""
---
**プロジェクト概要**:  
- 病院内のカテーテル在庫管理を想定したAIモデルの運用イメージデモアプリです。  
- フェーズ3 (モデル開発・検証) & フェーズ4 (本番運用・MLOps) を簡単にデモし、ビッグデータ想定の在庫分析例やエグゼクティブレポート画面を紹介します。

""")

# 3つのタブ: (1) フェーズ3 & 4 デモ, (2) ビッグデータ在庫分析, (3) ダッシュボード・レポートまとめ
tabs_main = st.tabs(["フェーズ3 & 4 デモ", "ビッグデータ在庫分析", "エグゼクティブレポート"])
